...
def extract_organism(pub: Dict) -> str:
    ...
    organism_patterns = {
        "Homo sapiens": ["homo sapiens", "human", "patient", "hek293", "hela", "a549", "mcf7",
                         "jurkat", "hepg2", "u2os", "k562", "clinical"],
...
def extract_organism_part(pub: Dict) -> str:
    ...
    cell_line_organ = {
        ...
        r"\bsw480\b": "colon", r"\bsw620\b": "colon",
        r"\ba549\b": "lung",
        r"\bmcf[\s-]?7\b": "breast",
        r"\blncap\b": "prostate",
...
def extract_disease(pub: Dict) -> str:
    ...
    disease_patterns = {
        ...
        "Prostate carcinoma": [r"\bprostate\s+carcinoma", r"\bprostate\s+cancer"],
...
def extract_modifications(pub: Dict) -> List[str]:
    ...
    if len(mods) == 1 and mods[0] == "Oxidation":
        return mods
...
def extract_material_type(pub: Dict) -> str:
    ...
    # also after cell line checks so A549/HCT116/etc. studies return 'cell' not 'tissue')
    organ_title_pat = r"\b(?:liver|hepatic|brain|cerebral|heart|cardiac|kidney|renal|lung|pulmonary|colon|colorectal|prostate|pancreas|pancreatic|ovary|ovarian|muscle|spleen|retina|synovial|thyroid|adrenal|testis|testicular|uterus|uterine|bladder)\b"
    ...
    # Specific tissue variants (before generic 'tissue')
    if re.search(r"\bbulk\s+tissue\b", text):
        return "bulk tissue"

    # Tissue/biopsy indicators
    tissue_patterns = [
        r"\btissue\s+(?:sample|biopsy|section|specimen)",
        r"\bbiopsy\b", r"\btumor\s+tissue\b", r"\btissue\s+lysate\b",
        r"\bffpe\b", r"\bformalin[- ]fixed\b",
        r"\btissue\s+proteom", r"\borgan\s+proteom",
    ]
    for p in tissue_patterns:
        if re.search(p, text):
            return "tissue"

    if re.search(r"\blysate\b", text):
        return "lysate"

    # Generic cell culture (after lysate check, so "yeast cell lysate" → lysate)
    if re.search(r"\bcell\s+(?:culture|pellet|extract|suspension)\b", text):
        return "cell"

    if re.search(r"\b(?:organ|tissue)\b", text) and not re.search(r"\bcell\b", text):
        return "organism part"

    if re.search(r"\bsingle[- ]cell\b", text):
        return "cell"
...
def extract_acquisition_method(pub: Dict, raw_files: List[str] = None) -> str:
    ...
    # Default to DDA (most common in proteomics)
    return "Data-Dependent Acquisition"
...
def extract_developmental_stage(pub: Dict) -> str:
    ...
    # Clinical studies with human patients are typically adult
    if re.search(r"\bpatient[s]?\b|\bdoner[s]?\b|\bdonor[s]?\b|\bvolunteer[s]?\b", abstract):
...
def parse_filename_bioreplicate(fname: str) -> str:
    ...
    # rep1, rep2, R1, R2 pattern
    m = re.search(r'[_\-](?:rep|r)(\d+)(?:[_\-]|$)', stem, re.IGNORECASE)
    if m:
        return m.group(1)

    # repN at end of stem (e.g. _rep1 or -rep2)
    m = re.search(r'[_\-](?:rep)(\d+)$', stem, re.IGNORECASE)
    if m:
        return m.group(1)
...
def extract_pxd_metadata(
    ...
    for raw_file in unique_files:
        ...
        if rows_per_file > 1 and label_type != "label free":
